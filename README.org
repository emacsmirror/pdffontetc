...
[[https://melpa.org/#/pdffontetc][file:https://melpa.org/packages/pdffontetc-badge.svg]]
* PDF Fonts Etc.
...
Now, initial experimental support has also been added showing information from [[https://exiftool.org][exiftool]], which can access additional potentially important additional PDF information, like PDF/UA.[fn:1]
...
You can install this package from [[https://melpa.org/#/][Melpa]], including via `use-package`, e.g.:
...
*** With bespoke package managers
The easiest if you're using Emacs 30 or above is the [[https://git.savannah.gnu.org/cgit/emacs.git/commit/?id=2ce279680bf9c1964e98e2aa48a03d6675c386fe][built-in]] ~vc-use-package~. 
...
If you use an alternative/supplement package manager already, you probably know what you're doing. But some tips for different options:
...
** Miscellaneous Notes
*** The pdffonts help key (available in emacs after the package is loaded via ~pdffontetc-pdffonts-man-help~):
...
(for more details on PDF font types, see [[https://en.wikipedia.org/wiki/PostScript_fonts][Wikpedia on "PostScript fonts"]])
...
- [[https://codeberg.org/MonadicSheep/emacs-reader][emacs-reader]]: aims to support a full range of "document"  formats, including not only PDF, but also esp. EPUB, MOBI, FB2, XPS/OpenXPS, CBZ (with lower priority support for Liberoffice-type Open Document Formats - ODT, ODS, ODP, ODG); focussing on taking advantage of dynamic (native) modules of Emacs, and ultimately [[https://github.com/minad/emacs-canvas-patch][upcoming patches to Emacs' canvas rendering]] and drawing on [[https://codeberg.org/MonadicSheep/pale][PALE]] (a Picture and Animation Library for Emacs, which attempts to implement real-time graphics in Emacs [see the [[https://github.com/minad/doom-on-emacs][Doom on Emacs demo]]]) 
...
[fn:1] E.g, for looking if a document is at least formally PDF/UA-1 compliant, check:
  - =Part=: Should be =1= (indicating PDF/UA-1).
  - =PDFUAVersion=: Often shows the specific version of the standard.
  - =AMD=: (Amendment) may show any revisions to the standard.
  - =Trapped=: Must be False for PDF/UA-1 compliance.
  - =Title=: A document title is mandatory for PDF/UA-1; if this is empty, it fails compliance. 
  (The fact that =exiftool -Part= returned ~1~ confirms a file has the PDF/UA-1 identifier embedded only means the file claims to be compliant. A structural validator like [[https://pdfa.org/product/pac/][PAC 2024]] is still needed to ensure the tags and reading order are actually correct, and even that doesn't guarantee the document is fully implements everything that is suggested for accessibility.)
